...
class ViewController: UIViewController {
    ...
    func drawAll() {
       ...
       // ctx.globalCompositeOperation = CanvasCompositeOperationType.Xor
       // arcToAnimationExample(ctx: ctx)
        
        ballExample(ctx: ctx)
        //draw1()
       /* draw2()
        let ctx = canvas3?.getContext(type: "2d") as! CanvasRenderingContext2D
        _ = canvas3?.ensureIsContextIsCurrent()
        drawArcMDN(ctx:ctx)*/
    ...
    func requestAnimationFrame(ctx: CanvasRenderingContext2D, toLoop: @escaping (Int) -> Void){
        let date = Date()
        ...
        let timeToCall = max(0, 16 - (currTime - lastTime))
        let timer = Timer(fire: Date(), interval: Double(timeToCall), repeats: false) { (currentTimer) in
          toLoop(timeToCall)
    ...
    func getCanvasWidth(canvas: Canvas) -> Float32 {
        return Float32(canvas.frame.width * UIScreen.main.scale)
    }
    
    func getCanvasHeight(canvas: Canvas) -> Float32 {
        return Float32(canvas.frame.height * UIScreen.main.scale)
    }
    ...
    class KeyValue{
        ...
        init(x: Float, y: Float) {
            self.x = x * Float(UIScreen.main.scale)
            self.y = y * Float(UIScreen.main.scale)
        }
    }
    ...
    ball.vx = -ball.vx;
        ...
        requestAnimationFrame(ctx:ctx){ (timer) in
            self.draw(ctx: ctx)
        }
    }
    ...
    func createRadialGradientExample(ctx: CanvasRenderingContext2D){
        ...
        let gradient = ctx.createRadialGradient(x0: 110,float: 90,r0: 30, x1: 100,y1: 100,r1: 70);
    ...
    func x2Clock(ctx: CanvasRenderingContext2D){
        ...
        ctx.clearRect(x: 0, y: 0, width: Float(150 * scale), height: Float(150 * scale));
        ctx.translate(x: Float(75 * scale), y: Float(75 * scale));
        ctx.scale(x: 0.4, y: 0.4);
        ctx.rotate(angle: -PI / 2);
        ctx.strokeStyle = CanvasColorStyle.Color(color: .black);
        ctx.fillStyle = CanvasColorStyle.Color(color: .white);
        ctx.lineWidth = Float(8  * scale);
        ctx.lineCap = "round";
        ...
        for _ in 0...11{
            ...
            ctx.moveTo(x: Float(100 * scale), y: 0);
            ctx.lineTo(x: Float(120 * scale), y: 0);
            ctx.stroke();
        }
        ctx.restore();
        ...
        // Minute marks
        ctx.save();
        ctx.lineWidth = Float(5 * scale);
        for i in 0...59{
            if (i % 5 != 0) {
                ctx.beginPath();
                ctx.moveTo(x: Float(117 * scale), y: 0);
                ctx.lineTo(x: Float(120 * scale), y: 0);
                ctx.stroke();
            }
        ...
        ctx.lineWidth = Float(14 * scale);
        ctx.beginPath();
        ctx.moveTo(x: Float(-20 * scale), y: 0);
        ctx.lineTo(x: Float(80 * scale), y: 0);
        ctx.stroke();
        ctx.restore();
        ...
        ctx.lineWidth = Float(10 * scale);
        ctx.beginPath();
        ctx.moveTo(x: Float(-28 * scale), y: 0);
        ctx.lineTo(x: Float(112 * scale), y: 0);
        ctx.stroke();
        ctx.restore();
        ...
        ctx.lineWidth = Float(6 * scale);
        ctx.beginPath();
        ctx.moveTo(x: Float(-30 * scale), y: 0);
        ctx.lineTo(x: Float(83 * scale), y: 0);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(x: 0, y: 0, radius: Float(10 * scale), startAngle: 0, endAngle: PI * 2, anticlockwise: true);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(x: Float(95 * scale), y: 0, radius: Float(10 * scale), startAngle: 0, endAngle: PI * 2, anticlockwise: true);
        ctx.stroke();
        ctx.fillStyle = CanvasColorStyle.Color(color: UIColor(red: 0, green: 0, blue: 0, alpha: 0));
        ctx.arc(x: 0, y: 0, radius: Float(3 * scale), startAngle: 0, endAngle: PI * 2, anticlockwise: true);
        ctx.fill();
        ctx.restore();
        ...
        ctx.arc(x: 0, y: 0, radius: Float(142 * scale), startAngle: 0, endAngle: PI * 2, anticlockwise: true);
        ctx.stroke();
...
//        ctx.strokeStyle =   CanvasColorStyle.Color(color: UIColor(hue: 2/360, saturation: CGFloat(colorIndex/100), brightness: 0.5, alpha: (CGFloat(1.2-dist/minDist))))
    ...
    particle_draw(ctx: ctx);
        requestAnimationFrame(ctx:ctx){(_) in self.animloop(ctx: ctx)}
    }
    ...
    func runEllipse(ctx: CanvasRenderingContext2D) {
        ...
        requestAnimationFrame(ctx:ctx) { (_) in
    ...
    ctx.beginPath();
            ctx.rect(x: (x * 25) * Float(UIScreen.main.scale), y: (y * 25) * Float(UIScreen.main.scale), width: Float(24 * UIScreen.main.scale) , height: Float(24 * UIScreen.main.scale));
